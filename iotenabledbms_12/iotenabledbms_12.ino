...
void loop() {
   temperature();
    ...
    if (isLithiumProgram) {
        ...
        // float time = timeRemaining_LI(6100,SOH_LI);
        ...
        float voltage = socLib.readBatteryVoltage(VOLTAGE_PIN);
        // analogReadVoltage();
        Serial.print("Voltage :");
...
float analogReadVoltage() {
    ...
    float leadVoltage = adcVoltage * (R1 + R2) / R2;
...
float timeRemaining_LI( float totalCapacity,float soc,float voltage)
{
  ...
  if (current > 0) {
    ...
    lcd.print(remainingTime, 2);
    lcd.print("hr");
    int sop = readSOP(power,6800);
        if (Firebase.setInt(fbdo, F("batterydata/lithiumion/sop"), sop)) {
...
int readSOP(int currentPower,int totalPower )
{
  int power= (currentPower/totalPower)*100;
  return power;
}